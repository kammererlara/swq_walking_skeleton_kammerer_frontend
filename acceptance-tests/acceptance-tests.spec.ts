...
test('should load RelocationRequestForm and validate field types', async ({
  page,
}) => {
...
test('should display a success message and clear fields on successful submission', async ({
  page,
}) => {
  ...
  const requestPromise = page.waitForRequest(
    'http://localhost:8080/requestForRelocationSupport',
  );
